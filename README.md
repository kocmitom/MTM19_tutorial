...
We are using following naming convention. *Parent* is the model trained on a high-resource language pair. *Child* is the low-resource model created by fine-tuning parent with child training data.

In this tutorial, we use English-to-Czech as a parent model and English-to-Estonian as a child model.

This transfer learning tutorial has following steps:

1. Obtain parallel data
2. Prepare vocabulary (shared between parent and child)
3. Train parent model (English-to-Czech)
4. Prepare data for child (English-to-Estonian)
...
Tensor2tensor uses 4 different folders. `t2t_datagen` is used for temporal storage of data. `t2t_data` is used for preprocessed training corpora and vocabularies. `t2t_train` is used to store trained models. And `codebase` is used for user specific definition of problems. The folders can be named differently.

## Preparation of vocabulary

Wordpieces (or BPE) can segment any text. We can use parent vocabulary to segment child training data. However, it is better to use vocabulary that contains language specific subwords. In order to have vocabulary, that contain parent and child subwords, we need to prepare shared vocabulary in advance of parent model training.

### Task 1

Take *equal amount* of parallel data from each training corpora and combine them together into one file called `mixed.txt`. The transfer learning isgoing to work even when the vocabulary is not balanced, however you get worse performance.
...
This creates vocabulary `vocab.cseten.wp` in `t2t_data` with 32k subwords. The vocabulary contains subwords from all languages of parent and child (Czech, English, Estonian). Note that we could improve performance of child model by making the vocabulary smaller as noted by Sennrich and Zhang (2019).
...
Now as you prepared vocabulary, you can preprocess dataset with the wordpieces and start parent training. The training would take several days/weeks, thus we skip this step and download already trained parent model:
...
This parent model has been trained for 1 700 000 steps (approximatelly 25 days on single GPU).

Note that we need to override generated vocabulary from previous step with the one used for training the parent model. In order to make sure that the vocabulary has exactly the same subwords as the trained parent model.
...
We evaluate the translation by sacreBLEU. This tool automatically download WMT testset and evaluate the translation on a correct testset:
...
Before training child model, Tensor2Tensor needs to preprocess training data. For this we are going to specify our own training problem and preprocess data.
We define our own problem in directory `codebase` and then preprocess data.

### Task 2

Open file `codebase/child.py`. We have a template of problem, that extends standard TranslateProblem of T2T. We need to specify where the training and development data are and also spefify the vocabulary.

The template does not work, we need to first modify rows 8 and 9 by providing the correct name of the child training corpora. T2T will look for the corpora in folder `t2t_datagen` where temporary files are stored. 

Furthermore, provide a correct name of vocabulary we generated earlier on the row 16.

The last step is to register the problem for tensor2tensor by adding following line into `codebase/__init__.py`.

```
import . from child
```

We preprocess the corpus by following command:
...
The last step is to train the child model. It is done by running following script. Before running, look inside to know what it does.

The script calls `t2t-trainer` with default parameters. We use *transformer_big_single_gpu*, other parameters are set exactly as for the parent model. You can increase batch size if you have access to GPU with bigger memory.
...
After the training, the final model can be tested by following command:

```
MODEL=child FILE=t2t_datagen/test-newstest2018-enet.src ./translate_file.sh
```

You can change the script's beam size to higher value (for example 4) and obtain better output.
...
After only 4000 steps (several hours of training) we obtain a score of 12.5 BLEU and after 30 000 steps you should get around a score 16.4 BLEU.

To summarize, we trained English-to-Estonian neural machine transltion with only 50k training sentences. If trained from scratch without the transfer learning and hyperparameter tuning, we would obtain roughly 5 BLEU points. 
